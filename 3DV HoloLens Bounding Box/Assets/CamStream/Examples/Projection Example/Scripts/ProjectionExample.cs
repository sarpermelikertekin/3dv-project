    ...
    private void OnFrameSampleAcquired(VideoCaptureSample sample)
    {
        ...
        UnityEngine.WSA.Application.InvokeOnAppThread(() =>
        {
            if (!stopVideo)
            {
                ...
                Vector2 centerImageCoord = new Vector2(_resolution.width / 2, _resolution.height / 2);
                ...
                Vector3 objDirection = LocatableCameraUtils.PixelCoordToWorldCoord(camera2WorldMatrix, projectionMatrix, _resolution, centerImageCoord);
                // define the mask
                int physicsLayer = 31;
                int PhysicsRaycastMask = 1 << physicsLayer;

                if (Physics.Raycast(headPosition, objDirection, out objHitInfo, 30.0f, PhysicsRaycastMask))
                {
                    depth = objHitInfo.distance;
                }
